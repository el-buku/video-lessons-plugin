...
class ShopifyStoreClient():
    ...
    def create_redirect(self, path: str, target: str) -> dict:
        call_path=f'redirects.json'
        method='POST'
        payload={
            'redirect':{
                'path':path,
        ...
        redirect_response= self.authenticated_shopify_call(call_path=call_path, method=method, payload=payload)
